...
class SkipGramModel:
    ...
    def _create_embedding(self):
        """
        Step 2 + 3: define weights and embedding lookup. In word2vec, it's actually
        the weights that we care about
        """
    ...
    def _create_loss(self):
        ...
        with tf.name_scope('loss'):
            ...
            nce_weight_init = tf.truncated_normal_initializer(
            ...
            raw_loss = tf.nn.nce_loss(weights=nce_weight,
                                      biases=nce_bias,
                                      labels=self.target_words,
                                      inputs=self.embed,
                                      num_sampled=self.num_sampled,
                                      num_classes=self.vocab_size)
            self.loss = tf.reduce_mean(raw_loss, name='loss')
